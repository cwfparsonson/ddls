...
class SipMlOpPartitioner:
    def __init__(self, min_op_run_time_quantum=10e-6, max_partitions_per_op: int = 8):
    ...
    def get(self, 
        ...
        for job in jobs:
            ...
            for forward_op_id in forward_graph.nodes:
                ...
                worker_type = 'A100'
                num_partitions = int(max(1, min(math.ceil(math.ceil(forward_graph.nodes[forward_op_id]['compute_cost'][worker_type] / self.min_op_run_time_quantum) / 2) * 2, self.max_partitions_per_op)))
