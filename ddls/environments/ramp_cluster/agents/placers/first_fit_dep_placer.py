...
# class FirstFitDepPlacer(Placer):
            ...
            # if job_id in new_job_op_placements:
                ...
                # for dep in job.computation_graph.edges:
                    ...
                    # if parent_node != child_node and size > 0:
                        ...
                        # path, channel_num = self._get_valid_path_channel_num(cluster, dep, job, channel_ids_used_for_other_jobs, new_job_op_placements)
                        ...
                        # else:
                            ...
                            # for idx in range(len(path) - 1):
                                ...
                                # _channel_ids_used_for_other_jobs.add(gen_channel_id(src, dst, channel_num))
                    # else:
    ...
    # def _get_valid_path_channel_num(self, cluster, dep, job, channel_ids_used_for_other_jobs, new_job_op_placements):
        ...
        # for path in paths:
            # for channel_num in range(cluster.topology.num_channels):
                # is_valid = self._check_path_channel_valid(path, channel_num, job, dep, cluster, channel_ids_used_for_other_jobs)
                # if is_valid:
    ...
    # def _check_path_channel_valid(self, path, channel_num, job, dep, cluster, channel_ids_used_for_other_jobs):
        # is_valid = True
        ...
        for job_id, job in cluster.job_queue.jobs.items():
            ...
            if job_id in new_job_op_placements:
                ...
                for dep_id in job.computation_graph.edges:
                    ...
                    if parent_node != child_node and size > 0:
                        # non-zero tensor with parent and child on different workers becomes a network flow -> must be placed
                        path, channel_num = self._get_valid_path_channel_num(cluster, dep_id, job, channel_ids_used_for_other_jobs, new_job_op_placements)
                        # print(f'\nValid path chosen: {path} | channel: {channel_num}')
                        if path is None:
                            # no valid placement for this flow
                            try:
                            ...
                            # job_to_dep_to_path_to_channel_num[job_id][dep_id][json.dumps(path)] = channel_num
                            for idx in range(len(path) - 1):
                                src, dst = (path[idx], path[idx+1])
                                channel_id = gen_channel_id(src, dst, channel_num)
